    ...
    class Connection
      ...
      module Requests
        ...
        def get_raw(endpoint, path, data = {}, headers = {})
          ...
          request(:raw => true) do
            token(endpoint).get "#{Dropbox::API::Config.prefix}#{URI.parse(URI.escape(path, Regexp.new("[^/a-z0-9]", true)))}?#{URI.parse(URI.encode(query))}", headers
          end
        end
        ...
        def post(endpoint, path, data = {}, headers = {})
          request do
            token(endpoint).post "#{Dropbox::API::Config.prefix}#{URI.parse(URI.escape(path, Regexp.new("[^/a-z0-9]", true)))}", data, headers
          end
        end

        def put(endpoint, path, data = {}, headers = {})
          request do
            token(endpoint).put "#{Dropbox::API::Config.prefix}#{URI.parse(URI.escape(path, Regexp.new("[^/a-z0-9]", true)))}", data, headers
          end
        end
